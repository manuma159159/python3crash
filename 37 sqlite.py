...
# sql 작성 - 아래 함수는 비추. -SQL injection 공격 위험 존재
...
conn.commit() # 테이블에 값을 저장하기 위해 반드시 commit 해야함
...
# 매개변수 named parameter placeholer(:이름)를 이용해서 실제 값 지정
sql = ' update member set passwd = :pwd, name = :name, email = :email where userid = :uid '

# named parameter placeholer 사용하는 경우
# 실제 값은 dict형으로 정의해야 함
