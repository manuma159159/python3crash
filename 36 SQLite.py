...
# 데이터베이스 생성 : .open 디비명.db     *************** 작업을 남기려면 꼭 이걸 하고 해야함 ***************
# .open bigdata.db
...
# pragma table_info('member'); sql이기 때문에 꼭 ;붙이자.
...
# ('abc123','987xyz','abc123','abc123@naver.com');
...
# .mode tabs
# .import tsv파일명 테이블명
# .import c:/Java/zipcode_20130201/zipcode_20130201(2).txt zipcode2013

# 단, import시 csv, tsv 파일의 인코딩은 utf-8 이어야함!!
# sqlite 나갈 때는 .quit

# 파이썬으로 sqllite 다루기 1 - select
import sqlite3
...
sql = 'select * from zipcode2013 where dong like "대방%"'
cursor.execute(sql)


# 반복문을 이용해서 커서에 저장된 결과집합으로 부터 한행씩 읽어서 처리
...
sql = ' select job_id, count(first_name) emps from employees group by job_id '
cursor.execute(sql)
...
cursor.close()
conn.close()          # conn이 먼저 켜졌으니 나중에 꺼짐.
...
sql =(' select Country, Medal, count(Country) as cnt from summermedals  group by Country, Medal order by cnt desc ')
cursor.execute(sql)
...
sql = ' select EMPLOYEE_ID, LAST_NAME, SALARY, JOB_ID, department_name from EMPLOYEES inner join main.departments d '\
      ' on EMPLOYEES.DEPARTMENT_ID = d.department_id limit 0 , 15 '
cursor.execute(sql)
result = ''
while True:
    row = cursor.fetchone()
